...
def check_setup():
    import yaml
    cfg = yaml.safe_load((q.ROOT.parent / "board" / "configs.yaml").read_text(encoding="utf-8"))
...
def check_clean():
    ...
    pats = [
        ...
        (r"\.agents|\brules/|\bboard/|\bsource/|rulebook-draft|glossary-draft|\.yaml|\.md\b", "путь к рабочему файлу"),
    ]
    for pat, why in pats:
        for m in re.finditer(pat, text, re.I):
            ctx = text[max(0, m.start()-60):m.end()+60].replace("\n", " ")
