# CODE-01 — собрать исходники правил в `source/`
...
**Ничего вне `rules/` не создавать, не переименовывать и не удалять.**
...
```
source/<original|redesign>/<SRC-ID>__<короткое-имя>__<ГГГГ-ММ-ДД>.<ext>
```
...
→ `source/gdocs/SRC-DRAFT__rules-draft__2026-09-10.md`
...
Извлечь текст из трёх PDF в `source/gdocs/pdf/` (исходники не трогать, только читать):

| Файл | Куда |
|---|---|
| `source/gdocs/pdf/Нефтяные войны (приложения) - Особые фабрики.pdf` | `source/gdocs/SRC-APP-FACTORY__special-factories__2026-09-10.md` |
| `source/gdocs/pdf/Нефтяные войны (наемники).pdf` | `source/gdocs/SRC-APP-MERC__mercenaries__2026-09-10.md` |
| `source/gdocs/pdf/Нефтяные войны (приложения) - Уникальные полковники.pdf` | `source/gdocs/SRC-APP-COLONEL__unique-colonels__2026-09-10.md` |
...
|---|---|---|
| Рулбук, старая вёрстка | `1wCEAJffAPaKYqctvlrKnjseLUr2s2psy` | `source/cthulhu-wars/SRC-RB-OLD__cw-rulebook-old-layout__2026-09-10.pdf` |
| Рулбук, новая вёрстка | `1320UvIs5J2c8kUHPIuItAThc7f1NGwD6` | `source/cthulhu-wars/SRC-RB-NEW__cw-rulebook-new-layout__2026-09-10.pdf` |
...
1. Excel с таблицей балансных правок:
   ...
   → `source/cthulhu-wars/SRC-ERRATA-XLSX__balance-changes__2026-09-10.xlsx` + `.md` с содержимым листа.
2. **Ultimate Errata Pack** — PDF лежит по ссылке из поста Kickstarter:
   ...
   `source/cthulhu-wars/SRC-ERRATA-PACK__ultimate-errata-pack__2026-09-10.pdf` + `.md`.
   Если Kickstarter требует логин — **не обходить**, зафиксировать как недоступное.

Таблица балансных правок уже сохранена вручную в
`source/cthulhu-wars/SRC-ERRATA__balance-changes-table__2026-09-10.md` — её перезаписывать не надо,
...
→ `source/cthulhu-wars/SRC-FAQ__cw-rules-faq__2026-09-10.md`
...
- `48001060995-cthulhu-wars-product-list` → `source/cthulhu-wars/SRC-PRODUCTS__cw-product-list__2026-09-10.md`
...
→ `source/cthulhu-wars/SRC-NECRO__rulebook__2026-09-10.md` (или `.json`)
...
- отчёты появятся в `rules/reports/`, `registry.db` — в `rules/registry/`.

## Шаг 8. Отчёт

Дописать в конец `source/README-источники.md` раздел:
...
- Не трогать ничего вне `rules/`, кроме чтения `source/gdocs/pdf/*.pdf`.
- Не обходить логины, капчи и платные стены. Недоступное — в отчёт списком.
- Не коммитить ничего в `print/.git`.
