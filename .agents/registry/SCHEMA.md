...
**Подреестр карты — `map/`, своя схема `map/SCHEMA.md`.** Реконструкция
...
(`MR-*`, `ME-*`, `MC-*`, `MAP-*`). Собирается своим скриптом `../tools/map.py`,
...
- id: SRC-RB2                     # SRC-###
  ...
  url: null
  local_path: sources/original/... # относительно .agents/, либо null
...
реестры проекта. `tools/registry.py` его не валидирует и в `registry.db`
