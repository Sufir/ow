...
и отчёт `../../out/11-map.md` собираются скриптом `../../tools/map.py` и руками
не правятся.

Общие соглашения те же, что в `../SCHEMA.md`: UTF-8, отступ 2 пробела, даты
...
записью в `glyphs.yaml`, а не полем области. `tools/map.py` сшивает их сам.
...
против которого `tools/map.py verify` проверяет собранный граф:
...
regions:
  - id: OW-L5-NAMW              # OW-<тот же SHORT>
    ...
    psd_layer: Сев. Америка - запад   # имя слоя в Карта/карта 5.psd
...
`../factions.yaml` (там текст и обоснование); здесь они привязаны к ID областей
...
`../factions.yaml` (`mapping`). Находки карты поэтому носят префикс `MF-###`.
